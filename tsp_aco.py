...
if __name__ == "__main__":
	...
	cidades = pd.read_csv('out1.txt', header=None).values

	# cria um grafo passando o número de vértices
	grafo = Grafo(num_vertices=len(cidades))

	d = {}
	for i in range(len(cidades)):
		d[str(cidades.tolist()[i])] = i + 1

	for origem in d.keys():
		for destino in d.keys():
			if origem != destino:
				distance = math.sqrt(pow(eval(destino)[0]-eval(origem)[0], 2) + pow(eval(destino)[1]-eval(origem)[1], 2))
				grafo.adicionarAresta(d[origem], d[destino], distance)
				#print d[origem], d[destino], distance

	# cria uma instância de ACO
	aco = ACO(grafo=grafo, num_formigas=grafo.num_vertices, alfa=1.0, beta=1.0, iteracoes=1000, evaporacao=0.5)
	# roda o algoritmo
	solucao, custo = aco.rodar()
	#print solucao
	#print custo
	...
	for s in solucao:
		for key, value in d.items():
			if value == s:
				result.append(eval(key))
	#print result

	plt.plotar2(result, custo, 1000)
